...
<ul>
    ...
    <li>
        ...
        <p>SpiritX的原始动机与React是基本一致的，即构建一个易于维护的大型动态系统。这里的动态是指系统的状态会随时间的变化而改变，而易于维护可以理解为更新系统内部的一个构成要素的代价主要集中在实现这一构成要素本身。基于此目标，SpiritX的出发点是构建一个尽可能通用的系统模型。我们认为React在这一道路上已经迈出了非常坚实的一步（我们也注意到NG2.0在这一方向上的努力）。而SpiritX过滤了一些与此模型无关的特性，实现了一个更为纯粹的以组件为中心的前端解决方案。相对于React而言，SpiritX在组件化的方向上更为彻底——</p>
		<p><font size=5>Web组件并不是作为widget嵌套在html中，而是作为整个web app的基本构成要素。</font></p>
        ...
        <p>最后，SpiritX更为长远的目标是实现完全组件化、本地化、UI自适应的web app。关于完全组件化的其中一个激进思想是抛弃html作为组件的模板。但就当前阶段而言，我们不会做更多的扩展了。</p>
		</p>
    </li>
</ul>
