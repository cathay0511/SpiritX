...
<ul>
    <li>
        <p>什么是SpiritX？</p>
        <p>广义的SpiritX指代一种Web前端解决方案。当前阶段主要包括一个以组件为中心的JavaScript框架SpiritXJS，以及基于该框架的基础组件库。</p>
    </li>
    <li>
        <p>为什么使用SpiritX？</p>
        <p>目前web app的前端实现有两种较为主流的解决方案：一种是以Angular1X为代表的以模板为中心的解决方案，另一种则是以React为代表的以组件为中心的解决方案。<p/>
		<p>以模板为中心的解决方案是web app在石器时代的产物——即将大量业务逻辑附加在一个服务于数据展现的标记文档中(虽然如此，Angular1X依然是非常成功的)。用于小型网站的开发问题不大，对于大型网站尤其是具有复杂交互逻辑的网站则会显得非常难以维护。其中一个主要的原因是系统的内部构成要素不具有明确的边界。<p/>
        ...
        <p>SpiritX的原始动机与React是基本一致的，即构建一个易于维护的大型动态系统。这里的动态是指系统的状态会随时间的变化而改变，而易于维护可以理解为更新系统内部的一个构成要素的代价主要集中在实现这一构成要素本身。基于此目标，SpiritX的出发点是构建一个尽可能通用的系统模型。我们认为React在这一道路上已经迈出了非常坚实的一步（我们也注意到Angular2X在这一方向上的努力）。而SpiritX过滤了一些与此模型无关的特性，实现了一个更为纯粹的以组件为中心的前端解决方案。相对于React而言，SpiritX在组件化的方向上更为彻底——</p>
		<h3>Web组件并不是作为widget嵌套在html中，而是作为整个web app的基本构成要素。</h3>
		<p>这是SpiritX当前阶段最为重要的思想。我们用实践证明，这一解决方案所具有的普遍性——其实现主要关注三个问题：
        <ul>
          <li>组件的构造</li>
          <li>组件生命周期</li>
          <li>组件间通信</li>
        ...
        <p>另外，除了模块加载使用了sea以及一些dom操作借助了jQuery，所有SpiritX的对象完全使用原生的javascript实现。这意味着SpiritX的组件可以方便的为其它框架所使用。</p>
        <p>最后，SpiritX更为长远的目标是实现完全组件化、本地化、UI自适应的web app。关于完全组件化的其中一个激进思想是抛弃html作为组件的模板。但就当前阶段而言，我们不会在此就上述topic作更多的展开了。</p>
		</p>
    </li>
</ul>
